...
output_file = os.path.join(output_dir, 'TransactionServicePilotUnitest_20260118.xml')
...
for f in files:
    try:
        tree = ET.parse(f)
        ts = tree.getroot()
        # collect failure/error totals from file attributes if present
        try:
        ...
        for tc in ts.findall('.//testcase'):
            ...
            status = 'passed'
            message = ''
            # check failure or error children
            fail = tc.find('failure')
            err = tc.find('error')
            if fail is not None:
                status = 'failed'
                message = fail.attrib.get('message', '').strip() + '\n' + (fail.text or '').strip()
            elif err is not None:
                status = 'error'
                message = err.attrib.get('message', '').strip() + '\n' + (err.text or '').strip()
        ...
        # ignore parse errors, but record
        cases.append({
...
# try to read jacoco coverage percentage
coverage_pct = 'N/A'
...
if os.path.exists(jacoco_xml):
    try:
        ...
        # find INSTRUCTION counter or LINE
        instr = None
        for c in counters:
